...
class Games(commands.Cog):
    ...
    async def rockPaperScissors(self, ctx):
        ...
        rpsInitial.add_field(name=":question:     :vs:     :question:", value="Click on a reaction to make your choice.")
        ...
        try:
            ...
            rpsFinal.add_field(name=f"{reaction}     :vs:     {computerChoice}", value=f"You {result}!")
    ...
    async def ticTacToe(self, ctx): 
        ...
        freeCells = 9
        usersTurn = True
        tttBoard = [[":one:", ":two:", ":three:"], [":four:", ":five:", ":six:"], [":seven:", ":eight:", ":nine:"]]
        validPositions = ("1", "2", "3", "4", "5", "6", "7", "8", "9")
        winner = ""

        # Select difficulty
        tttEmbed = discord.Embed(title="Tic Tac Toe", description="Select a difficulty")
        ...
        while not winner and freeCells > 0:
            # Show Board
            tttEmbed = discord.Embed(title="Tic Tac Toe", description=getBoard(tttBoard))
            ...
            if usersTurn:
                ...
                while not validMove:
                    try:
                        await ctx.send("Which square do you want to move to (1-9)?")
                        msg = await self.client.wait_for('message', check=lambda msg: msg.author==ctx.author and msg.content in validPositions, timeout = 60.0)
                    except asyncio.TimeoutError:
                        return
                    ...
                    if tttBoard[row][col] == ":x:" or tttBoard[row][col] == ":o:":
                        continue
                    else:
                        tttBoard[row][col] = ":x:"
                        validMove = True
    ...
    async def connectFour(self, ctx): 
        ...
        if len(mentions) != 0:
            if mentions[0] == player1:
                ...
                player2 = mentions[0]       # Small chance it won't take the first mention, discord limitation
        else: 
            player2 = "Krulaina"
        ...
        # Unicode emojis (1-7)
        choices = ["{}\N{COMBINING ENCLOSING KEYCAP}".format(num) for num in range(1, 8)]

        # Show board
        c4Embed = discord.Embed(title="Connect 4", description=getBoard(c4Board), colour=discord.Colour.red())
        ...
        while not winner and freeCells > 0:
            ...
            if player1Turn:
                ...
                newEmbed = discord.Embed(title="Connect 4", description=getBoard(c4Board), colour=discord.Colour.red())
                newEmbed.set_footer(text="Click on a reaction to place a piece")
                ...
                while not validMove:
                    ...
                    try:
                        ...
                        await ctx.send(f"{player1} you took too long to respond, quitting game.")
                        newEmbed = discord.Embed(title="Connect 4", description=getBoard(c4Board))
                ...
                # Change embed colour to indicate turn
                newEmbed = discord.Embed(title="Connect 4", description=getBoard(c4Board), colour=discord.Colour.gold())
                newEmbed.set_footer(text="Click on a reaction to place a piece")
                await message.edit(embed=newEmbed)
                ...
                if player2 != "Krulaina":
                    ...
                    while not validMove:
                        ...
                        try:
                            ...
                            await ctx.send(f"{player2} you took too long to respond, quitting game.")
                            newEmbed = discord.Embed(title="Connect 4", description=getBoard(c4Board))
                            newEmbed.set_footer(text="Game timed out")
                            await message.edit(embed=newEmbed)
                            return
        ...
        # Game end message
        finalEmbed = discord.Embed(title="Connect 4", description=getBoard(c4Board), colour=discord.Colour.green())
